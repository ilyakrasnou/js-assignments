...
function extractNameFromTemplate(value) {
  const regex = /Hello, (?<firstName>.*) (?<lastName>.*)!/;
  const match = value.match(regex);
  return `${match.groups.firstName} ${match.groups.lastName}`;
}
...
function encodeToRot13(str) {
  ...
  function repclacer(match) {
    return rotated.charAt(original.indexOf(match));
  }

  const regex = new RegExp(`[${original}]`, 'g');
  return str.replace(regex, repclacer);
}
